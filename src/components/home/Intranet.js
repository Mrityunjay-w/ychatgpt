...
const Internal = () => {
    ...
    const InitializeWebChat = useCallback(async () => {
        ...
        const tokenEndpointURL = new URL('https://default0da2a83b13d94a35965fec53a220ed.9d.environment.api.powerplatform.com/powervirtualagents/botsbyschema/cr09e_ychatgptIntranet/directline/token?api-version=2022-03-01-preview');
    ...
    return (
        <>
            {/* <button onClick={handleChatbotToggle} className="open-chatbot-btn">
                {isChatbotActive ? 'Hide Chatbot' : 'Show Chatbot'}
            </button> */}

            {isChatbotActive && (
                <div className={`chatbot-popup ${isMinimized ? 'minimized' : ''}`}>
                    <div id="banner">
                        <h1 className="banner-title">Y-ChatGPT-Intranet</h1>
